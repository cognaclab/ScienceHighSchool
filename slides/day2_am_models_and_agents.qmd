---
title: "Welcome & Building Toy Worlds"
subtitle: "RIKEN Wako Science Camp 2026 — Day 2 morning"
author: "Computational Group Dynamics (COGNAC) Collaboration Unit, RIKEN CBS"
format:
  revealjs:
    theme: [default]
    slide-number: true
    embed-resources: true
    transition: slide
execute:
  echo: false
  warning: false
  message: false
---

```{=html}
<style>
.scotland-photo-grid {
  display: grid;
  grid-template-columns: repeat(2, minmax(0, 1fr));
  gap: 0.35rem 0.7rem;
}

.scotland-photo-grid .photo-card {
  text-align: center;
}

.scotland-photo-grid .photo-card p {
  margin: 0.12rem 0 0;
}

.scotland-photo-grid .photo-card p:first-child {
  line-height: 0;
}

.scotland-photo-grid .photo-card img {
  width: 100%;
  height: 130px;
  object-fit: cover;
  margin: 0;
}
</style>
```

## Welcome! ようこそ！ {.center}

**Linked hands-on notebook（演習用ノートブック）:**<br>
`tutorials/02_simulating_randomness_notebook.Rmd`

**You + us, three teaching days, one goal:**

build a tiny society inside a computer —
and present what it teaches us about *real* people

::: {.fragment}
We will work in English. We will help each other.
:::

## Who we are {.smaller}

**Computational Group Dynamics (COGNAC) Collaboration Unit**
RIKEN Center for Brain Science

We study how *groups* of humans and
animals learn and decide together.

::::: {.columns}
::: {.column width="33.3%"}
![](assets/wataru_toyokawa_image_02.png){height=95}

**Wataru Toyokawa**
:::
::: {.column width="33.3%"}
![](assets/Henri-Vandendriessche.jpg){height=95}

**Henri Vandendriessche**
:::
::: {.column width="33.3%"}
![](assets/alexandrawitt_web.jpg){height=95}

**Alexandra Witt**
:::
:::::

::::: {.columns}
::: {.column width="50%"}
![](assets/ryutaro-mori.jpg){height=95}

**Ryutaro Mori**
:::
::: {.column width="50%"}
![](assets/seiya-nakata.jpg){height=95}

**Seiya Nakata**
:::
:::::

## A little about me {.smaller}

::::: {.columns}
::: {.column width="32%"}
![](assets/wataru_toyokawa_headshot_crop.png){height=300 fig-alt="Portrait of Wataru Toyokawa"}
:::

::: {.column width="68%"}
**Wataru Toyokawa (豊川 航)**

- I grew up in **Sendai, Miyagi**.
- I first studied **fisheries science（水産学）** at Hokkaido University.
- Later, I became interested in how people learn and make decisions as a group.
- I completed my PhD at Hokkaido University's Graduate School of Letters（文学研究科）.
:::
:::::

::: {.notes}
Suggested script — edit it as you like:

"Hello. My name is Wataru Toyokawa. I grew up in Sendai. At university, I
first studied fisheries science. Later, I became interested in how people
learn and make decisions collectively as a group. Then I completed my PhD at Hokkaido University."
:::

## My research journey: Scotland {.smaller}

After my PhD, I spent **four years** at the School of Biology,
University of St Andrews in Scotland, as a JSPS postdoctoral fellow（学振特別研究員）.

::::: {.columns}
::: {.column width="24%"}
![](assets/UK-map_web.jpg){width=92% fig-alt="Map showing St Andrews in Scotland"}

**St Andrews, Scotland**
:::

::: {.column width="76%"}
::::: {.scotland-photo-grid}
::: {.photo-card}
![](assets/st-andrews_web.jpg){fig-alt="View of St Andrews"}

The university town
:::
::: {.photo-card}
![](assets/highland-cattle_web.jpg){fig-alt="Highland cattle"}

Highland cattle
:::
::: {.photo-card}
![](assets/wt-isleofskye_web.jpg){fig-alt="Wataru on the Isle of Skye"}

The Isle of Skye
:::
::: {.photo-card}
![](assets/malted-barley-tasting-at-distillery_web.jpg){fig-alt="Wataru smelling malted barley at a whisky distillery"}

A whisky distillery
:::
:::::
:::
:::::

::: {.notes}
Suggested script — add one personal detail:

"After my PhD, I moved to Scotland. I spent four years at the University of
St Andrews. I worked in the School of Biology. Scotland was such a beautiful country. If you have a chance, I would highly recommend to visit or study there!
In St Andrews, I studied hoe people learn socially, and how such 'social learning' shapes behaviour of the group."
:::

## My research journey: Germany {.smaller}

Next, I spent **five years** as a research scientist at the
University of Konstanz.

::::: {.columns}
::: {.column width="24%"}
![](assets/germany_web.jpg){width=95% fig-alt="Map showing Konstanz in Germany"}

**Konstanz, Germany**
:::

::: {.column width="76%"}
::::: {.columns}
::: {.column width="26%"}
![](assets/SUP_web.jpg){height=180 fig-alt="Stand-up paddleboarding near Konstanz"}

Life by the lake
:::
::: {.column width="37%"}
![](assets/cosmos2022_web.jpg){height=165 fig-alt="COSMOS summer school group in 2022"}

COSMOS 2022
:::
::: {.column width="37%"}
![](assets/cosmos2023_web.jpg){height=165 fig-alt="COSMOS summer school group in 2023"}

COSMOS 2023
:::
:::::
:::
:::::

::: {.notes}
Suggested script — add one personal detail:

"Next, I moved to Germany. I spent five years as a research scientist at the
University of Konstanz. Konstanz is beside a large lake, so I enjoyed life
near the water. I also helped teach students at the COSMOS summer school.
[Add one short research topic or favourite memory here.]"
:::


## Groups do amazing things

- A school of fish turns as one — **no leader**
- Ants find the shortest path to food — **no map**
- A crowd guesses the weight of an ox better than any expert
- A silly dance spreads to 100 million people in a week

::: {.fragment}
**Nobody is in charge. So who decides?**
:::

## The puzzle of collective behaviour（集団行動）

Individual rules → group patterns

| Individual does... | The group shows... |
|---|---|
| follow your neighbour | flocking, waves |
| join the longer queue | traffic jams, viral trends |
| copy the majority | fashion, price booms, culture |

The group behaviour is not written anywhere —
it **emerges**（創発する）.

## How can we study this?

Watching groups is not enough — we need to know **why**.

::: {.incremental}
- Can't interview a fish 🐟
- Can't rewind a stock market crash ⏪
- Can't ask 1000 people to repeat their lives 100 times
:::

::: {.fragment}
**Solution: build the society ourselves — in a computer.**
:::

## What is a model?

A map of Tokyo is not Tokyo.

A perfectly accurate map of Tokyo would have to be the same size as Tokyo, making it useless.

A map is *useful because* it leaves things out.

::: {.fragment}
A **model** = a deliberately simplified world
keeping only what we care about

A **simulation** = pressing **play** on a model
:::

## Our route through the camp

| When | What we will do |
|---|---|
| **Before camp** | meet R in Tutorial 1 |
| **Day 1** | arrive, meet the team, and get oriented |
| **Day 2 morning** | build toy worlds in Tutorial 2 |
| **Day 2 afternoon** | build a learning agent in Tutorial 3 |
| **Day 3 morning** | add social learning in Tutorials 4–5 |
| **Day 3 afternoon** | investigate your own group question |
| **Day 4** | finish, rehearse, and present like scientists |

## Draft timetable: Day 2 {.smaller}

**Tuesday, 28 July — Models and learning agents**

| Time | Activity | Facilitator |
|---|---|---|
| **10:00–10:30** | Say hello and get set up<br>Check that R and RStudio work (Tutorial 1 if needed) | Teaching team |
| **10:30–12:00** | Tutorial 2: Simulating randomness | Wataru |
| **12:00–13:00** | Lunch | |
| **13:00–14:30** | Tutorial 3: Reinforcement-learning agents | Henri *(to confirm)* |
| **14:30–16:00** | Hands-on work and discussion | Teaching team *(to confirm)* |


::: {.fragment}
This is our current plan. We may adjust it together.
:::

## Draft timetable: Day 3 {.smaller}

**Wednesday, 29 July — Social learning and group projects**

| Time | Activity | Facilitator |
|---|---|---|
| **10:00–11:30** | Tutorials 4–5: Social learning | Alex & Ryutaro *(to confirm)* |
| **11:30–12:00** | Discuss and choose a group project | Teaching team |
| **12:00–13:00** | Lunch | |
| **Afternoon** | Work on group projects | Teaching team |
| **17:30–19:00** | Reception | All |

::: {.fragment}
You will turn a question into a small simulation study.
:::

## Draft timetable: Day 4 {.smaller}

**Thursday, 30 July — Complete and share the project**

| Time | Activity |
|---|---|
| **Before 15:00** | Finish the analysis, prepare the presentation, and rehearse<br>*(exact schedule to confirm)* |
| **15:00** | Group presentation |

::: {.fragment}
The Day 4 preparation schedule is still open for discussion.
:::

## Our lab language: R

- Free software used by scientists everywhere
- You type commands, R answers, draws, simulates
- Tutorial 1 gave you the basics; today we use them to do science

```{r}
#| echo: true
sample(1:6, size = 10, replace = TRUE)   # R rolls 10 dice
```

::: {.fragment}
Every simulation this week is built from
little pieces of randomness like this.
:::

## Tutorial 2: three tools

You will build:

1. **Repeated experiments** — flip thousands of virtual coins
2. **A function** — give a useful set of commands a name
3. **A random walk** — make a path from uncertain steps

::: {.fragment}
The goal is not to memorise code.

The goal is to ask: **what pattern appears when simple rules repeat?**
:::

## Rules of the camp lab

1. **Type it yourself** — copy-paste teaches nothing
2. **Predict before you run** — then be surprised
3. **Errors are normal** — red text happens to professionals daily
4. **Ask anything** — asking is what researchers do all day

## Hands-on: Tutorial 2 {.center}

Open:

`tutorials/02_simulating_randomness_notebook.Rmd`

Work from top to bottom. **Predict before you run.**

Need an R refresher? Ask us, or check
`tutorials/01_welcome_to_r_notebook.Rmd`.
